...
#define MUST_BE_FIRST() do { if (!first) { errmsg = "invalid address"; return ERR; } } while (0)
...
#define GET_THIRD_ADDR(addr) { long ol1, ol2; ol1 = first_addr, ol2 = second_addr; if (extract_addr_range() < 0) return ERR; else if (addr_cnt == 0) { errmsg = "destination expected"; return ERR; } else if (second_addr < 0 || addr_last < second_addr) { errmsg = "invalid address"; return ERR; } addr = second_addr; first_addr = ol1, second_addr = ol2; }
...
#define GET_THIRD_ADDR(addr) { long ol1, ol2; ol1 = first_addr, ol2 = second_addr; if (extract_addr_range() < 0) return ERR; if (second_addr < 0 || addr_last < second_addr) { errmsg = "invalid address"; return ERR; } addr = second_addr; first_addr = ol1, second_addr = ol2; }
...
#define GET_COMMAND_SUFFIX() { int done = 0; do { switch(*ibufp) { case 'p': gflag |= GPR, ibufp++; break; case 'l': gflag |= GLS, ibufp++; break; case 'n': gflag |= GNP, ibufp++; break; default: done++; } } while (!done); if (*ibufp++ != '\n') { errmsg = "invalid command suffix"; return ERR; } }
