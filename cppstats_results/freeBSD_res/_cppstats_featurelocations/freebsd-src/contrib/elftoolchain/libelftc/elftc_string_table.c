...
#define ELFTC_STRING_TABLE_CLEAR_COMPACTION_FLAG(st) do { (st)->st_len &= ~ELFTC_STRING_TABLE_COMPACTION_FLAG; } while (0)


#define ELFTC_STRING_TABLE_SET_COMPACTION_FLAG(st) do { (st)->st_len |= ELFTC_STRING_TABLE_COMPACTION_FLAG; } while (0)


#define ELFTC_STRING_TABLE_UPDATE_LENGTH(st, len) do { (st)->st_len = ((st)->st_len & ELFTC_STRING_TABLE_COMPACTION_FLAG) | ((len) << 1); } while (0)
