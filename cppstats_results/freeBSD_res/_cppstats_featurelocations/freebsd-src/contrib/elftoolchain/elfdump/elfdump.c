...
#define DEFINE_SHFLAGS() DEFINE_SHF(WRITE) DEFINE_SHF(ALLOC) DEFINE_SHF(EXECINSTR) DEFINE_SHF(MERGE) DEFINE_SHF(STRINGS) DEFINE_SHF(INFO_LINK) DEFINE_SHF(LINK_ORDER) DEFINE_SHF(OS_NONCONFORMING) DEFINE_SHF(GROUP) DEFINE_SHF(TLS) DEFINE_SHF(COMPRESSED)
...
#undef DEFINE_SHF
#define DEFINE_SHF(N) if (shf & SHF_##N) strcat(flg, "SHF_" #N "|");
...
#define AC(CALL) do { if ((CALL)) { warnx("%s", archive_error_string(a)); return; } } while (0)
