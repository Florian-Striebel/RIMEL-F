...
(void) dbg->read(ds->ds_data, off, dwarf_size);
...
#define CHECK_TABLE_SIZE(x) do { if ((x) >= rt->rt3_reg_table_size) { DWARF_SET_ERROR(dbg, error, DW_DLE_DF_REG_NUM_TOO_HIGH); ret = DW_DLE_DF_REG_NUM_TOO_HIGH; goto program_done; } } while(0)
...
#define SET_BASE_OP(x) do { if (fop != NULL) fop[*count].fp_base_op = (x) >> 6; if (fop3 != NULL) fop3[*count].fp_base_op = (x) >> 6; } while(0)
...
#define SET_EXTENDED_OP(x) do { if (fop != NULL) fop[*count].fp_extended_op = (x); if (fop3 != NULL) fop3[*count].fp_extended_op = (x); } while(0)
...
#define SET_REGISTER(x) do { if (fop != NULL) fop[*count].fp_register = (x); if (fop3 != NULL) fop3[*count].fp_register = (x); } while(0)
...
#define SET_OFFSET(x) do { if (fop != NULL) fop[*count].fp_offset = (x); if (fop3 != NULL) fop3[*count].fp_offset_or_block_len = (x); } while(0)
...
#define SET_INSTR_OFFSET(x) do { if (fop != NULL) fop[*count].fp_instr_offset = (x); if (fop3 != NULL) fop3[*count].fp_instr_offset = (x); } while(0)
...
#define SET_BLOCK_LEN(x) do { if (fop3 != NULL) fop3[*count].fp_offset_or_block_len = (x); } while(0)
...
#define SET_EXPR_BLOCK(addr, len) do { if (fop3 != NULL) { fop3[*count].fp_expr_block = malloc((size_t) (len)); if (fop3[*count].fp_expr_block == NULL) { DWARF_SET_ERROR(dbg, error, DW_DLE_MEMORY); return (DW_DLE_MEMORY); } memcpy(&fop3[*count].fp_expr_block, (addr), (len)); } } while(0)
