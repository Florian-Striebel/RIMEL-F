...
#define CHECK_SYM_PRINT_DATA(p) (p->headp == NULL || p->sh_num == 0 || p->t_table == NULL || p->s_table == NULL || p->filename == NULL)
...
#define PRINT_DEMANGLED_NAME(FORMAT, NAME) do { char _demangled[DEMANGLED_BUFFER_SIZE]; if (nm_opts.demangle_type < 0 || elftc_demangle((NAME), _demangled, sizeof(_demangled), nm_opts.demangle_type) < 0) printf((FORMAT), (NAME)); else printf((FORMAT), _demangled); } while (0)
