...
#define DW_DEFAULT_OPTIONS (DW_A | DW_F | DW_I | DW_L | DW_O | DW_P | DW_R | DW_RR | DW_S)
...
#define PH_HDR "Type", "Offset", "VirtAddr", "PhysAddr", "FileSiz", "MemSiz", "Flg", "Align"

#define PH_CT phdr_type(re->ehdr.e_machine, phdr.p_type), (uintmax_t)phdr.p_offset, (uintmax_t)phdr.p_vaddr, (uintmax_t)phdr.p_paddr, (uintmax_t)phdr.p_filesz, (uintmax_t)phdr.p_memsz, phdr.p_flags & PF_R ? 'R' : ' ', phdr.p_flags & PF_W ? 'W' : ' ', phdr.p_flags & PF_X ? 'E' : ' ', (uintmax_t)phdr.p_align
...
#define S_HDR "[Nr] Name", "Type", "Addr", "Off", "Size", "ES", "Flg", "Lk", "Inf", "Al"

#define S_HDRL "[Nr] Name", "Type", "Address", "Offset", "Size", "EntSize", "Flags", "Link", "Info", "Align"

#define ST_HDR "[Nr] Name", "Type", "Addr", "Off", "Size", "ES", "Lk", "Inf", "Al", "Flags"

#define ST_HDRL "[Nr] Name", "Type", "Address", "Offset", "Link", "Size", "EntSize", "Info", "Align", "Flags"

#define S_CT i, s->name, section_type(re->ehdr.e_machine, s->type), (uintmax_t)s->addr, (uintmax_t)s->off, (uintmax_t)s->sz,(uintmax_t)s->entsize, section_flags(re, s), s->link, s->info, (uintmax_t)s->align



#define ST_CT i, s->name, section_type(re->ehdr.e_machine, s->type), (uintmax_t)s->addr, (uintmax_t)s->off, (uintmax_t)s->sz,(uintmax_t)s->entsize, s->link, s->info, (uintmax_t)s->align, section_flags(re, s)



#define ST_CTL i, s->name, section_type(re->ehdr.e_machine, s->type), (uintmax_t)s->addr, (uintmax_t)s->off, s->link, (uintmax_t)s->sz, (uintmax_t)s->entsize, s->info, (uintmax_t)s->align, section_flags(re, s)
...
#define REL_CT32 (uintmax_t)r.r_offset, (uintmax_t)r.r_info, elftc_reloc_type_str(re->ehdr.e_machine, ELF32_R_TYPE(r.r_info)), (uintmax_t)symval, symname


#define REL_CT64 (uintmax_t)r.r_offset, (uintmax_t)r.r_info, elftc_reloc_type_str(re->ehdr.e_machine, type), (uintmax_t)symval, symname
...
#define RELA_HDR "r_offset", "r_info", "r_type", "st_value", "st_name + r_addend"

#define RELA_CT32 (uintmax_t)r.r_offset, (uintmax_t)r.r_info, elftc_reloc_type_str(re->ehdr.e_machine, ELF32_R_TYPE(r.r_info)), (uintmax_t)symval, symname


#define RELA_CT64 (uintmax_t)r.r_offset, (uintmax_t)r.r_info, elftc_reloc_type_str(re->ehdr.e_machine, type), (uintmax_t)symval, symname
...
#define SAVE_VERSION_NAME(x, n, t) do { while (x >= re->ver_sz) { nv = realloc(re->ver, sizeof(*re->ver) * re->ver_sz * 2); if (nv == NULL) { warn("realloc failed"); free(re->ver); return; } re->ver = nv; for (i = re->ver_sz; i < re->ver_sz * 2; i++) { re->ver[i].name = NULL; re->ver[i].type = 0; } re->ver_sz *= 2; } if (x > 1) { re->ver[x].name = n; re->ver[x].type = t; } } while (0)
...
#if !defined(Tag_GNU_MIPS_ABI_FP)
...
#if !defined(Tag_GNU_Power_ABI_FP)
#define Tag_GNU_Power_ABI_FP 4
#endif

#if !defined(Tag_GNU_Power_ABI_Vector)
...
#define RESET_REGISTERS do { address = 0; file = 1; line = 1; column = 0; is_stmt = defstmt; } while(0)
