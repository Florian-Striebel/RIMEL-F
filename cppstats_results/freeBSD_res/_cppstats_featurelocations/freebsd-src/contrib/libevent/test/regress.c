...
#define setup_test(x) do { if (!in_legacy_test_wrapper) { TT_FAIL(("Legacy test %s not wrapped properly", x)); return; } } while (0)
...
#if defined(EVENT__HAVE_WAITPID_WITH_WNOWAIT)
wait_flags |= WNOWAIT;
#endif
...
#if defined(EVENT__HAVE_UNSETENV)
#define UNSETENV_OK
