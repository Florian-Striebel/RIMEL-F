...
char url_tmp[4096];
#define URI_PARSE(uri) evhttp_uri_parse_with_flags((uri), parse_flags)
...
#define BAD(s) do { if (URI_PARSE(s) != NULL) TT_FAIL(("Expected error parsing \"%s\"",s)); } while(0)




#define NCF(s) do { uri = URI_PARSE(s); if (uri != NULL && !nonconform) { TT_FAIL(("Expected error parsing \"%s\"",s)); } else if (uri == NULL && nonconform) { TT_FAIL(("Couldn't parse nonconformant URI \"%s\"", s)); } if (uri) { tt_want(evhttp_uri_join(uri, url_tmp, sizeof(url_tmp))); evhttp_uri_free(uri); } } while(0)
...
#define ENC(from,want,plus) do { s = evhttp_uriencode((from), -1, (plus)); tt_assert(s); tt_str_op(s,==,(want)); sz = -1; s2 = evhttp_uridecode((s), (plus), &sz); tt_assert(s2); tt_str_op(s2,==,(from)); tt_int_op(sz,==,strlen(from)); free(s); free(s2); s = s2 = NULL; } while (0)
...
#define DEC(from,want,dp) do { s = evhttp_uridecode((from),(dp),&sz); tt_assert(s); tt_str_op(s,==,(want)); tt_int_op(sz,==,strlen(want)); free(s); s = NULL; } while (0)
...
#define OLD_DEC(from,want) do { s = evhttp_decode_uri((from)); tt_assert(s); tt_str_op(s,==,(want)); free(s); s = NULL; } while (0)
...
#define HTTP_LEGACY(name) { #name, run_legacy_test_fn, TT_ISOLATED|TT_LEGACY, &legacy_setup, http_##name##_test }



#define HTTP_CAST_ARG(a) ((void *)(a))
#define HTTP_OFF_N(title, name, arg) { #title, http_##name##_test, TT_ISOLATED|TT_OFF_BY_DEFAULT, &basic_setup, HTTP_CAST_ARG(arg) }

#define HTTP_N(title, name, arg) { #title, http_##name##_test, TT_ISOLATED, &basic_setup, HTTP_CAST_ARG(arg) }

#define HTTP(name) HTTP_N(name, name, NULL)
#define HTTPS(name) { "https_" #name, https_##name##_test, TT_ISOLATED, &basic_setup, NULL }
