...
#define evbuffer_validate(buf) TT_STMT_BEGIN if (!evbuffer_validate_(buf)) TT_DIE(("Buffer format invalid")); TT_STMT_END
...
#define tt_line_eq(content) TT_STMT_BEGIN if (!cp || sz != strlen(content) || strcmp(cp, content)) { TT_DIE(("Wanted %s; got %s [%d]", content, cp, (int)sz)); } TT_STMT_END
...
#define tt_iov_eq(v, s) tt_int_op((v)->iov_len, ==, strlen(s)); tt_assert(!memcmp((v)->iov_base, (s), strlen(s)))
...
#define FREEZE_EQ(a, startcase, endcase) do { if (start) { tt_int_op((a), ==, (startcase)); } else { tt_int_op((a), ==, (endcase)); } } while (0)
...
#define ADDFILE_TEST(name, parameters) { name, test_evbuffer_add_file, TT_FORK|TT_NEED_BASE, &basic_setup, (void*)(parameters) }



#define ADDFILE_TEST_GROUP(name, parameters) ADDFILE_TEST(name "_sendfile", "sendfile " parameters), ADDFILE_TEST(name "_mmap", "mmap " parameters), ADDFILE_TEST(name "_linear", "linear " parameters)
