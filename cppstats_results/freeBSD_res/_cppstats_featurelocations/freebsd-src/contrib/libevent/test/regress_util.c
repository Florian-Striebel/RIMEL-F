...
{
#define TEST_TRIM(s, result) do { if (cp) mm_free(cp); cp = mm_strdup(s); tt_assert(cp); evutil_rtrim_lws_(cp); tt_str_op(cp, ==, result); } while(0)
...
#define RESET() do { logsev = 0; if (logmsg) free(logmsg); logmsg = NULL; } while (0)




#define LOGEQ(sev,msg) do { tt_int_op(logsev,==,sev); tt_assert(logmsg != NULL); tt_str_op(logmsg,==,msg); } while (0)





#if defined(CAN_CHECK_ERR)
