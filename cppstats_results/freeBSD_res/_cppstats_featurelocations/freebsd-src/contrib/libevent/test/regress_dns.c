...
dns_server_gethostbyname_cb, NULL);
resolve_addr.s_addr = htonl(0xc0a80b0bUL);
...
#if defined(EVENT_SET_MEM_FUNCTIONS_IMPLEMENTED)
...
#define IMPL_DBG_LEAK_RESUME(name, cancel, send_err_shutdown) static void test_dbg_leak_##name##_(void *env_) { dbg_leak_resume(env_, cancel, send_err_shutdown); }
...
#define DNS_LEGACY(name, flags) { #name, run_legacy_test_fn, flags|TT_LEGACY, &legacy_setup, dns_##name }
