...
#if defined(EVENT__HAVE_SYS_IOCTL_H)
#include <sys/ioctl.h>
#endif
#include <sys/queue.h>
...
#define ENCODE_INT_INTERNAL(data, number) do { int off = 1, nibbles = 0; memset(data, 0, sizeof(number)+1); while (number) { if (off & 0x1) data[off/2] = (data[off/2] & 0xf0) | (number & 0x0f); else data[off/2] = (data[off/2] & 0x0f) | ((number & 0x0f) << 4); number >>= 4; off++; } if (off > 2) nibbles = off - 2; data[0] = (data[0] & 0x0f) | ((nibbles & 0x0f) << 4); return ((off + 1) / 2); } while (0)
