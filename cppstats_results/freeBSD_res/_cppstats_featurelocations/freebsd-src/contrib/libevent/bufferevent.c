...
#define SCHEDULE_DEFERRED(bevp) do { if (event_deferred_cb_schedule_( (bevp)->bev.ev_base, &(bevp)->deferred)) bufferevent_incref_(&(bevp)->bev); } while (0)
...
#define BEV_TRIG_ALL_OPTS ( BEV_TRIG_IGNORE_WATERMARKS| BEV_TRIG_DEFER_CALLBACKS )
