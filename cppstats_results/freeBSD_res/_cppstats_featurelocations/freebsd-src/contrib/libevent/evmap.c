...
#define GET_IO_SLOT(x, map, slot, type) do { struct event_map_entry key_, *ent_; key_.fd = slot; ent_ = HT_FIND(event_io_map, map, &key_); (x) = ent_ ? &ent_->ent.type : NULL; } while (0);
...
#define GET_SIGNAL_SLOT_AND_CTOR(x, map, slot, type, ctor, fdinfo_len) do { if ((map)->entries[slot] == NULL) { (map)->entries[slot] = mm_calloc(1,sizeof(struct type)+fdinfo_len); if (EVUTIL_UNLIKELY((map)->entries[slot] == NULL)) return (-1); (ctor)((struct type *)(map)->entries[slot]); } (x) = (struct type *)((map)->entries[slot]); } while (0)
