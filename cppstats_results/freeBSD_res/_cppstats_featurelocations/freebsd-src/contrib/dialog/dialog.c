...
,o_yes_label
,o_yesno
#if defined(HAVE_WHIPTAIL)
,o_fullbutton
,o_topleft
#endif
#if defined(HAVE_XDIALOG)
,o_calendar
,o_dselect
,o_editbox
,o_fselect
...
#else
#define handle_leaks()
#define ignore_leak(n)
#endif

#define OptionChars "0123456789-abcdefghijklmnopqrstuvwxyz"
...
#define DisableNoTags() bool save_no_tags = dialog_vars.no_tags; bool save_no_items = dialog_vars.no_items; dialog_vars.no_tags = TRUE; dialog_vars.no_items = FALSE





#define RestoreNoTags() dialog_vars.no_tags = save_no_tags; dialog_vars.no_items = save_no_items
...
#define TraceTarget ((target == (char *) &dialog_state) ? "dialog_state" : "dialog_vars")



#define TraceBVal (bval ? "TRUE" : "FALSE")
#define TraceDone(fmt,value) DLG_TRACE(("#\t.. set %s.%s = %"fmt"\n", TraceTarget, data->name, value))

#define TraceLate(fmt,value) DLG_TRACE(("#\t.. defer setting %s = %"fmt"\n", data->name, value))
