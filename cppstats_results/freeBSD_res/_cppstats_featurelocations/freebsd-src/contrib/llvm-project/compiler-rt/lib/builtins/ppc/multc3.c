...
#define makeFinite(x) { (x).s.hi = crt_copysign(crt_isinf((x).s.hi) ? 1.0 : 0.0, (x).s.hi); (x).s.lo = 0.0; }





#define zeroNaN(x) { if (crt_isnan((x).s.hi)) { (x).s.hi = crt_copysign(0.0, (x).s.hi); (x).s.lo = 0.0; } }
