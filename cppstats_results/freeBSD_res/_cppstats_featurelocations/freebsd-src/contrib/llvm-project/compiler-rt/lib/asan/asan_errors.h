...
length1(length1_),
length2(length2_),
addr1_description(addr1, length1, false),
addr2_description(addr2, length2, false),
function(function_) {
char bug_type[100];
...
bp(bp_),
sp(sp_),
addr1_description(p1, 1, false),
addr2_description(p2, 1, false) {}
void Print();
};
...
#define ASAN_ERROR_DESCRIPTION_CONSTRUCTOR(name) ErrorDescription(Error##name const &e) : kind(kErrorKind##name) { internal_memcpy(&name, &e, sizeof(name)); }



#define ASAN_ERROR_DESCRIPTION_PRINT(name) case kErrorKind##name: return name.Print();
