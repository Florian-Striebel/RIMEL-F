...
#if !defined(ASAN_INTERCEPTORS_H)
...
#define ENSURE_ASAN_INITED() do { CHECK(!asan_init_is_running); if (UNLIKELY(!asan_inited)) { AsanInitFromRtl(); } } while (0)
...
#if ASAN_HAS_EXCEPTIONS && !SANITIZER_WINDOWS && !SANITIZER_SOLARIS && !SANITIZER_NETBSD
...
#if SANITIZER_LINUX && (defined(__arm__) || defined(__aarch64__) || defined(__i386__) || defined(__x86_64__) || SANITIZER_RISCV64)
...
#define ASAN_INTERCEPT_FUNC_VER_UNVERSIONED_FALLBACK(name, ver) do { if (!INTERCEPT_FUNCTION_VER(name, ver) && !INTERCEPT_FUNCTION(name)) VReport(1, "AddressSanitizer: failed to intercept '%s@@%s' or '%s'\n", #name, #ver, #name); } while (0)
