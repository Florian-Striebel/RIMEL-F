...
#define GET_STACK_TRACE_FATAL(thr, pc) VarSizeStackTrace stack; ObtainCurrentStack(thr, pc, &stack); stack.ReverseOrder();
...
#define HACKY_CALL(f) __asm__ __volatile__("sub $1024, %%rsp;" CFI_INL_ADJUST_CFA_OFFSET(1024) ".hidden " #f "_thunk;" "call " #f "_thunk;" "add $1024, %%rsp;" CFI_INL_ADJUST_CFA_OFFSET(-1024) ::: "memory", "cc");
