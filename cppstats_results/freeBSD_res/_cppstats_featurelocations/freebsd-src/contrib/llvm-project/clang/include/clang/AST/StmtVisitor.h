...
#if !defined(LLVM_CLANG_AST_STMTVISITOR_H)
...
#define DISPATCH(NAME, CLASS) return static_cast<ImplClass*>(this)->Visit ##NAME( static_cast<PTR(CLASS)>(S), std::forward<ParamTys>(P)...)
...
#define BINOP_FALLBACK(NAME) RetTy VisitBin ##NAME(PTR(BinaryOperator) S, ParamTys... P) { DISPATCH(BinaryOperator, BinaryOperator); }
...
#define CAO_FALLBACK(NAME) RetTy VisitBin ##NAME(PTR(CompoundAssignOperator) S, ParamTys... P) { DISPATCH(CompoundAssignOperator, CompoundAssignOperator); }
...
#define UNARYOP_FALLBACK(NAME) RetTy VisitUnary ##NAME(PTR(UnaryOperator) S, ParamTys... P) { DISPATCH(UnaryOperator, UnaryOperator); }
