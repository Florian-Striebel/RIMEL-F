...
#if !defined(LLVM_CLANG_LIB_SEMA_TREETRANSFORM_H)
...
#define STMT(Node, Parent) LLVM_ATTRIBUTE_NOINLINE StmtResult Transform##Node(Node *S);


#define VALUESTMT(Node, Parent) LLVM_ATTRIBUTE_NOINLINE StmtResult Transform##Node(Node *S, StmtDiscardKind SDK);


#define EXPR(Node, Parent) LLVM_ATTRIBUTE_NOINLINE ExprResult Transform##Node(Node *E);
...
#define CLAUSE_CLASS(Enum, Str, Class) LLVM_ATTRIBUTE_NOINLINE OMPClause *Transform##Class(Class *S);
...
if (!SemaRef.isAcceptableTagRedeclaration(Tag, Kind, false,
IdLoc, Id)) {
...
Expr *SubExpr) {
return getSema().BuildUnaryOp(nullptr, OpLoc, Opc, SubExpr);
}
...
return getSema().ActOnArraySubscriptExpr(nullptr, LHS,
LBracketLoc, RHS,
RBracketLoc);
}
...
return getSema().ActOnOMPIteratorExpr(nullptr, IteratorKwLoc,
LLoc, RLoc, Data);
}
...
nullptr, Callee, LParenLoc, Args, RParenLoc, ExecConfig);
}
...
Expr *LHS, Expr *RHS) {
return getSema().BuildBinOp(nullptr, OpLoc, Opc, LHS, RHS);
}
...
TSInfo, LParenLoc, None, RParenLoc, false);
}
...
SS, NameInfo, IsAddressOfOperand, nullptr, RecoveryTSI);
}
...
#define VALUESTMT(Node, Parent) case Stmt::Node##Class: return getDerived().Transform##Node(cast<Node>(S), SDK);
...
#define GEN_CLANG_CLAUSE_CLASS
#define CLAUSE_CLASS(Enum, Str, Class) case Enum: return getDerived().Transform##Class(cast<Class>(S));
...
ExprResult Result =
IsCall ? getDerived().TransformInitializer(Inputs[I], false)
...
ObjectType);
if (SemaRef.BuildCXXNestedNameSpecifier(nullptr, IdInfo, false,
...
SS.Extend(SemaRef.Context, SourceLocation(), TL,
Q.getLocalEndLoc());
break;
}
...
Output = TemplateArgumentLoc(TemplateArgument(NewT, true),
TemplateArgumentLocInfo());
else
...
#define ABSTRACT_TYPELOC(CLASS, PARENT)
#define TYPELOC(CLASS, PARENT) case TypeLoc::CLASS: return getDerived().Transform##CLASS##Type(TLB, T.castAs<CLASS##TypeLoc>());
...
return QualType();
SizeResult =
SemaRef.ActOnFinishFullExpr(SizeResult.get(), false);
if (SizeResult.isInvalid())
return QualType();
...
OldParm, indexAdjustment, None, false);
}
...
#define ATTR(X)
#define PRAGMA_SPELLING_ATTR(X) case attr::X: return getDerived().Transform##X##Attr(cast<X##Attr>(R));
...
switch (getSema().CheckMicrosoftIfExistsSymbol(nullptr, SS, NameInfo)) {
...
NameInfo, true, ULE->isOverloaded(),
...
true, ULE->isOverloaded(), Decls.begin(), Decls.end()));
} else
UnresolvedReductions.push_back(nullptr);
}
...
TT.getSema().Context, nullptr,
MapperIdScopeSpec.getWithLocInContext(TT.getSema().Context),
MapperIdInfo, true, ULE->isOverloaded(), Decls.begin(),
Decls.end()));
} else {
...
return getDerived().RebuildArraySubscriptExpr(
LHS.get(),
E->getLHS()->getBeginLoc(), RHS.get(), E->getRBracketLoc());
}
...
E->getLParenLoc(), NewT,
E->getInitializer()->getEndLoc(), Init.get());
}
...
E->getBeginLoc(), PlacementArgs,
E->getBeginLoc(), E->getTypeIdParens(), AllocType,
...
Sema::ContextRAII SavedContext(getSema(), Body, false);
...
return TransformDependentScopeDeclRefExpr(E, false,
nullptr);
}
...
QualifierLoc, TemplateKWLoc, NameInfo, nullptr,
...
T, E->getBeginLoc(), Constructor, E->isElidable(), Args,
...
if (Body.isInvalid()) {
SavedContext.pop();
getSema().ActOnLambdaError(E->getBeginLoc(), nullptr,
true);
return ExprError();
}
...
if (TransformOverloadExprDecls(Old, false, R))
return ExprError();
...
SemaRef.ActOnBlockStart(E->getCaretLocation(), nullptr);
...
extParamInfos)) {
getSema().ActOnBlockError(E->getCaretLocation(), nullptr);
return ExprError();
}
...
getSema().ActOnTemplateName(nullptr, SS, TemplateKWLoc,
...
nullptr, SS, TemplateKWLoc, Name, ParsedType::make(ObjectType),
false, Template, AllowInjectedClassName);
return Template.get();
}
...
return SemaRef.checkPseudoObjectAssignment(nullptr, OpLoc, Opc,
First, Second);
...
getSema().ActOnCapturedRegionStart(Loc, nullptr,
