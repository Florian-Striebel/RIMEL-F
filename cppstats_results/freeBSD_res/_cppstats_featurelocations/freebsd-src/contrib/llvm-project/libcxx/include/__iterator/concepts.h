...
#if !defined(_LIBCPP___ITERATOR_CONCEPTS_H)
...
*__o = _VSTD::forward<_Tp>(__t);
*_VSTD::forward<_Out>(__o) = _VSTD::forward<_Tp>(__t);
const_cast<const iter_reference_t<_Out>&&>(*__o) = _VSTD::forward<_Tp>(__t);
const_cast<const iter_reference_t<_Out>&&>(*_VSTD::forward<_Out>(__o)) = _VSTD::forward<_Tp>(__t);
};
