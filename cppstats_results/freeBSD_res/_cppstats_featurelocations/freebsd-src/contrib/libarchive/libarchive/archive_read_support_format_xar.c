...
#if DEBUG_PRINT_TOC
#define PRINT_TOC(d, outbytes) do { unsigned char *x = (unsigned char *)(uintptr_t)d; unsigned char c = x[outbytes-1]; x[outbytes - 1] = 0; fprintf(stderr, "%s", x); fprintf(stderr, "%c", c); x[outbytes - 1] = c; } while (0)
