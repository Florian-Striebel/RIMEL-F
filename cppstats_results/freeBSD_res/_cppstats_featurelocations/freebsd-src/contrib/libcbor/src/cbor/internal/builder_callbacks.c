...
#define CHECK_RES(ctx, res) do { if (res == NULL) { ctx->creation_failed = true; return; } } while (0)
...
#define PUSH_CTX_STACK(ctx, res, subitems) do { if (_cbor_stack_push(ctx->stack, res, subitems) == NULL) { cbor_decref(&res); ctx->creation_failed = true; } } while (0)
