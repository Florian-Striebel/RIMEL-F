...
#define BE64(p) ( (CAST(uint64_t, (p)->hq[0])<<56)| (CAST(uint64_t, (p)->hq[1])<<48)| (CAST(uint64_t, (p)->hq[2])<<40)| (CAST(uint64_t, (p)->hq[3])<<32)| (CAST(uint64_t, (p)->hq[4])<<24)| (CAST(uint64_t, (p)->hq[5])<<16)| (CAST(uint64_t, (p)->hq[6])<<8)| (CAST(uint64_t, (p)->hq[7])))
...
#define LE64(p) ( (CAST(uint64_t, (p)->hq[7])<<56)| (CAST(uint64_t, (p)->hq[6])<<48)| (CAST(uint64_t, (p)->hq[5])<<40)| (CAST(uint64_t, (p)->hq[4])<<32)| (CAST(uint64_t, (p)->hq[3])<<24)| (CAST(uint64_t, (p)->hq[2])<<16)| (CAST(uint64_t, (p)->hq[1])<<8)| (CAST(uint64_t, (p)->hq[0])))
...
#define LE32(p) ( (CAST(uint32_t, (p)->hl[3])<<24)| (CAST(uint32_t, (p)->hl[2])<<16)| (CAST(uint32_t, (p)->hl[1])<<8)| (CAST(uint32_t, (p)->hl[0])))




#define BE32(p) ( (CAST(uint32_t, (p)->hl[0])<<24)| (CAST(uint32_t, (p)->hl[1])<<16)| (CAST(uint32_t, (p)->hl[2])<<8)| (CAST(uint32_t, (p)->hl[3])))




#define ME32(p) ( (CAST(uint32_t, (p)->hl[1])<<24)| (CAST(uint32_t, (p)->hl[0])<<16)| (CAST(uint32_t, (p)->hl[3])<<8)| (CAST(uint32_t, (p)->hl[2])))
...
#define SEXT(s,v,p) ((s) ? CAST(intmax_t, CAST(int##v##_t, p)) : CAST(intmax_t, CAST(uint##v##_t, p)))
...
#define DO_CVT2(fld, type) if (m->num_mask) switch (m->mask_op & FILE_OPS_MASK) { case FILE_OPADD: p->fld += CAST(type, m->num_mask); break; case FILE_OPMINUS: p->fld -= CAST(type, m->num_mask); break; case FILE_OPMULTIPLY: p->fld *= CAST(type, m->num_mask); break; case FILE_OPDIVIDE: if (CAST(type, m->num_mask) == 0) return -1; p->fld /= CAST(type, m->num_mask); break; }
