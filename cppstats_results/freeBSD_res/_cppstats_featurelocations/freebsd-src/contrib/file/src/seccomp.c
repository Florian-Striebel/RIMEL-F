...
#define DENY_RULE(call) do if (seccomp_rule_add (ctx, SCMP_ACT_KILL, SCMP_SYS(call), 0) == -1) goto out; while (0)




#define ALLOW_RULE(call) do if (seccomp_rule_add (ctx, SCMP_ACT_ALLOW, SCMP_SYS(call), 0) == -1) goto out; while (0)





#define ALLOW_IOCTL_RULE(param) do if (seccomp_rule_add(ctx, SCMP_ACT_ALLOW, SCMP_SYS(ioctl), 1, SCMP_CMP(1, SCMP_CMP_EQ, (scmp_datum_t)param, (scmp_datum_t)0)) == -1) goto out; while (0)
