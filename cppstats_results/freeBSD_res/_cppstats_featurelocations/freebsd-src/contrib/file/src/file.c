...
#include <unistd.h>
#include <string.h>
#if defined(RESTORE_TIME)
#if (__COHERENT__ >= 0x420)
#include <sys/utime.h>
#else
#if defined(USE_UTIMES)
#include <sys/time.h>
#else
#include <utime.h>
#endif
...
#define USAGE "Usage: %s [-" FILE_FLAGS "] [--apple] [--extension] [--mime-encoding]\n" " [--mime-type] [-e <testname>] [-F <separator>] " " [-f <namefile>]\n" " [-m <magicfiles>] [-P <parameter=value>] [--exclude-quiet]\n" " <file> ...\n" " %s -C [-m <magicfiles>]\n" " %s [--help]\n"
...
#define OPT(shortname, longname, opt, def, doc) {longname, opt, NULL, shortname},

#define OPT_LONGONLY(longname, opt, def, doc, id) {longname, opt, NULL, id},
...
"\n", stdout);
#define OPT(shortname, longname, opt, def, doc) fprintf(stdout, " -%c, --" longname, shortname), docprint(doc, def);


#define OPT_LONGONLY(longname, opt, def, doc, id) fprintf(stdout, " --" longname), docprint(doc, def);
