...
#define CDF_TOLE8(x) (CAST(uint64_t, NEED_SWAP ? _cdf_tole8(x) : CAST(uint64_t, x)))

#define CDF_TOLE4(x) (CAST(uint32_t, NEED_SWAP ? _cdf_tole4(x) : CAST(uint32_t, x)))

#define CDF_TOLE2(x) (CAST(uint16_t, NEED_SWAP ? _cdf_tole2(x) : CAST(uint16_t, x)))

#define CDF_TOLE(x) (sizeof(x) == 2 ? CDF_TOLE2(CAST(uint16_t, x)) : (sizeof(x) == 4 ? CDF_TOLE4(CAST(uint32_t, x)) : CDF_TOLE8(CAST(uint64_t, x))))
...
#define extract_catalog_field(t, f, l) if (b + l + sizeof(cep->f) > eb) { cep->ce_namlen = 0; break; } memcpy(&cep->f, b + (l), sizeof(cep->f)); ce[i].f = CAST(t, CDF_TOLE(cep->f))
