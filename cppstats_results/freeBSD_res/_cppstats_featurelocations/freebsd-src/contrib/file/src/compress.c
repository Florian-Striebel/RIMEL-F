...
#if defined(DEBUG)
int tty = -1;
#define DPRINTF(...) do { if (tty == -1) tty = open("/dev/tty", O_RDWR); if (tty == -1) abort(); dprintf(tty, __VA_ARGS__); } while (0)
...
#define METH_LZMA 13
#define METH_ZLIB 14
{ { .magic = "\037\235" }, 2, gzip_args, NULL },


{ { .magic = "\037\235" }, 2, uncompress_args, NULL },
{ { .magic = "\037\213" }, 2, gzip_args, do_zlib },
{ { .magic = "\037\236" }, 2, gzip_args, NULL },
{ { .magic = "\037\240" }, 2, gzip_args, NULL },

{ { .magic = "\037\036" }, 2, gzip_args, NULL },
{ { .magic = "PK\3\4" }, 4, gzip_args, NULL },

{ { .magic = "BZh" }, 3, bzip2_args, do_bzlib },
{ { .magic = "LZIP" }, 4, lzip_args, NULL },
{ { .magic = "\3757zXZ\0" },6, xz_args, NULL },
{ { .magic = "LRZI" }, 4, lrzip_args, NULL },
{ { .magic = "\004\"M\030" },4, lz4_args, NULL },
{ { .magic = "\x28\xB5\x2F\xFD" }, 4, zstd_args, NULL },
{ { .func = lzmacmp }, -13, xz_args, NULL },
#if defined(ZLIBSUPPORT)
{ { .func = zlibcmp }, -2, zlib_args, NULL },
#endif
};
