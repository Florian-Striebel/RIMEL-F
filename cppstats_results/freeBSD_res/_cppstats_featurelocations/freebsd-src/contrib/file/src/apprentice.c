...
ptr++;
#define CHECKLEN() do { for (len = cnt = 0; isdigit(CAST(unsigned char, *ptr)); ptr++, cnt++) len = len * 10 + (*ptr - '0'); if (cnt > 5 || len > 1024) goto toolong; } while (0)
