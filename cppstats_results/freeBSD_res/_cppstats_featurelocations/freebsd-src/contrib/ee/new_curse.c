...
Noblock = TRUE;
#if defined(SYS5)
Gterminal.c_cc[VTIME] = 0;
Gterminal.c_lflag &= ~ICANON;
Gterminal.c_lflag &= ~ECHO;
#endif
Count = 1;
Found = FALSE;
...
#if defined(SYS5)
value = ioctl(0, TCGETA, &Saved_tty);
#else
...
#if defined(SYS5)
value = ioctl(0, TCSETA, &Saved_tty);
#else
...
struct _line *
Insert_line(row, end_row, window)
int row;
int end_row;
WINDOW *window;
{
...
struct _line *
Delete_line(row, end_row, window)
int row;
int end_row;
WINDOW *window;
{
...
void
Char_del(line, attrib, offset, maxlen)
char *line;
char *attrib;
int offset;
int maxlen;
...
void
Char_ins(line, attrib, newc, newatt, offset, maxlen)
char *line;
char *attrib;
char newc;
char newatt;
