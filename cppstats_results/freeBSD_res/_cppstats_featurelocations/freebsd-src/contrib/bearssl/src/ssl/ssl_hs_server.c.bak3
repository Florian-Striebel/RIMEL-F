...
#define T0_ENTER(ip, rp, slot) do { const unsigned char *t0_newip; uint32_t t0_lnum; t0_newip = &t0_codeblock[t0_caddr[(slot) - T0_INTERPRETED]]; t0_lnum = t0_parse7E_unsigned(&t0_newip); (rp) += t0_lnum; *((rp) ++) = (uint32_t)((ip) - &t0_codeblock[0]) + (t0_lnum << 16); (ip) = t0_newip; } while (0)
...
#define T0_DEFENTRY(name, slot) void name(void *ctx) { t0_context *t0ctx = ctx; t0ctx->ip = &t0_codeblock[0]; T0_ENTER(t0ctx->ip, t0ctx->rp, slot); }
...
#define T0_ROLL(x) do { size_t t0len = (size_t)(x); uint32_t t0tmp = *(dp - 1 - t0len); memmove(dp - t0len - 1, dp - t0len, t0len * sizeof *dp); *(dp - 1) = t0tmp; } while (0)





#define T0_SWAP() do { uint32_t t0tmp = *(dp - 2); *(dp - 2) = *(dp - 1); *(dp - 1) = t0tmp; } while (0)




#define T0_ROT() do { uint32_t t0tmp = *(dp - 3); *(dp - 3) = *(dp - 2); *(dp - 2) = *(dp - 1); *(dp - 1) = t0tmp; } while (0)





#define T0_NROT() do { uint32_t t0tmp = *(dp - 1); *(dp - 1) = *(dp - 2); *(dp - 2) = *(dp - 3); *(dp - 3) = t0tmp; } while (0)





#define T0_PICK(x) do { uint32_t t0depth = (x); T0_PUSH(T0_PEEK(t0depth)); } while (0)



#define T0_CO() do { goto t0_exit; } while (0)
