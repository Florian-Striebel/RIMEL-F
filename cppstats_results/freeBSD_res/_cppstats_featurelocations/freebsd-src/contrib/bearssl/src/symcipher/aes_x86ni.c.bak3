...
#define KEXP128(k, i, rcon) do { k = expand_step128(k, _mm_aeskeygenassist_si128(k, rcon)); sk[i] = k; } while (0)




#define KEXP192(i, rcon1, rcon2) do { sk[(i) + 0] = t1; sk[(i) + 1] = t3; t2 = _mm_aeskeygenassist_si128(t3, rcon1); expand_step192(&t1, &t2, &t3); sk[(i) + 1] = _mm_castpd_si128(_mm_shuffle_pd( _mm_castsi128_pd(sk[(i) + 1]), _mm_castsi128_pd(t1), 0)); sk[(i) + 2] = _mm_castpd_si128(_mm_shuffle_pd( _mm_castsi128_pd(t1), _mm_castsi128_pd(t3), 1)); t2 = _mm_aeskeygenassist_si128(t3, rcon2); expand_step192(&t1, &t2, &t3); } while (0)
...
#define KEXP256(i, rcon) do { sk[(i) + 0] = t3; t2 = _mm_aeskeygenassist_si128(t3, rcon); expand_step256_1(&t1, &t2); sk[(i) + 1] = t1; expand_step256_2(&t1, &t3); } while (0)
