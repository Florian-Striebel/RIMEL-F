...
#define SHA2_STEP(A, B, C, D, E, F, G, H, j) do { uint32_t T1, T2; T1 = H + BSG2_1(E) + CH(E, F, G) + K[j] + w[j]; T2 = BSG2_0(A) + MAJ(A, B, C); D += T1; H = T1 + T2; } while (0)
...
#define SHA2_MEXP1(pc) do { W[pc] = br_dec32be(buf + ((pc) << 2)); } while (0)



#define SHA2_MEXP2(pc) do { W[(pc) & 0x0F] = SSG2_1(W[((pc) - 2) & 0x0F]) + W[((pc) - 7) & 0x0F] + SSG2_0(W[((pc) - 15) & 0x0F]) + W[(pc) & 0x0F]; } while (0)





#define SHA2_STEPn(n, a, b, c, d, e, f, g, h, pc) do { uint32_t t1, t2; SHA2_MEXP ##n(pc); t1 = h + BSG2_1(e) + CH(e, f, g) + K[pcount + (pc)] + W[(pc) & 0x0F]; t2 = BSG2_0(a) + MAJ(a, b, c); d += t1; h = t1 + t2; } while (0)
...
#define SHA2_STEP1(a, b, c, d, e, f, g, h, pc) SHA2_STEPn(1, a, b, c, d, e, f, g, h, pc)

#define SHA2_STEP2(a, b, c, d, e, f, g, h, pc) SHA2_STEPn(2, a, b, c, d, e, f, g, h, pc)
