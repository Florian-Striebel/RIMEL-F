...
#define FOR_EACH_HOOK_BEGIN(hooks_internal_ptr) for (int for_each_hook_counter = 0; for_each_hook_counter < HOOK_MAX; for_each_hook_counter++) { bool for_each_hook_success = seq_try_load_hooks( (hooks_internal_ptr), &hooks[for_each_hook_counter]); if (!for_each_hook_success) { continue; } if (!(hooks_internal_ptr)->in_use) { continue; }
...
#define HOOK_PROLOGUE if (likely(atomic_load_u(&nhooks, ATOMIC_RELAXED) == 0)) { return; } bool *in_hook = hook_reentrantp(); if (*in_hook) { return; } *in_hook = true;
