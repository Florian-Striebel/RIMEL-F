...
#define CTL_GET(n, v, t) do { size_t sz = sizeof(t); xmallctl(n, (void *)v, &sz, NULL, 0); } while (0)




#define CTL_M2_GET(n, i, v, t) do { size_t mib[CTL_MAX_DEPTH]; size_t miblen = sizeof(mib) / sizeof(size_t); size_t sz = sizeof(t); xmallctlnametomib(n, mib, &miblen); mib[2] = (i); xmallctlbymib(mib, miblen, (void *)v, &sz, NULL, 0); } while (0)
...
#define CTL_M2_M4_GET(n, i, j, v, t) do { size_t mib[CTL_MAX_DEPTH]; size_t miblen = sizeof(mib) / sizeof(size_t); size_t sz = sizeof(t); xmallctlnametomib(n, mib, &miblen); mib[2] = (i); mib[4] = (j); xmallctlbymib(mib, miblen, (void *)v, &sz, NULL, 0); } while (0)
...
#define OP(counter, counter_type, human, derived, base_counter) col = &col_##counter_type[k_##counter_type]; ++k_##counter_type; emitter_col_init(col, row); col->justify = emitter_justify_right; col->width = derived ? 8 : WIDTH_##counter_type; col->type = emitter_type_title; col->str_val = human;
...
#define EMITTER_TYPE_uint32_t emitter_type_uint32
#define EMITTER_TYPE_uint64_t emitter_type_uint64
#define OP(counter, type, human, derived, base_counter) if (!derived) { col = &col_##type[k_##type]; ++k_##type; emitter_json_kv(emitter, #counter, EMITTER_TYPE_##type, (const void *)&col->bool_val); }
...
#define COL(row_name, column_name, left_or_right, col_width, etype) emitter_col_t col_##column_name; emitter_col_init(&col_##column_name, &row_name); col_##column_name.justify = emitter_justify_##left_or_right; col_##column_name.width = col_width; col_##column_name.type = emitter_type_##etype;
...
#define COL_HDR(row_name, column_name, human, left_or_right, col_width, etype) COL(row_name, column_name, left_or_right, col_width, etype) emitter_col_t header_##column_name; emitter_col_init(&header_##column_name, &header_##row_name); header_##column_name.justify = emitter_justify_##left_or_right; header_##column_name.width = col_width; header_##column_name.type = emitter_type_title; header_##column_name.str_val = human ? human : #column_name;
...
#define GET_AND_EMIT_ALLOC_STAT(small_or_large, name, valtype) CTL_M2_GET("stats.arenas.0." #small_or_large "." #name, i, &small_or_large##_##name, valtype##_t); emitter_json_kv(emitter, #name, emitter_type_##valtype, &small_or_large##_##name); col_count_##name.type = emitter_type_##valtype; col_count_##name.valtype##_val = small_or_large##_##name;
...
#define GET_AND_EMIT_MEM_STAT(stat) CTL_M2_GET("stats.arenas.0."#stat, i, &stat, size_t); emitter_json_kv(emitter, #stat, emitter_type_size, &stat); mem_count_title.str_val = #stat":"; mem_count_val.size_val = stat; emitter_table_row(emitter, &mem_count_row);
...
#define CONFIG_WRITE_BOOL(name) do { CTL_GET("config."#name, &bv, bool); emitter_kv(emitter, #name, "config."#name, emitter_type_bool, &bv); } while (0)
...
#define OPT_WRITE(name, var, size, emitter_type) if (je_mallctl("opt."name, (void *)&var, &size, NULL, 0) == 0) { emitter_kv(emitter, name, "opt."name, emitter_type, &var); }
...
#define OPT_WRITE_MUTABLE(name, var1, var2, size, emitter_type, altname) if (je_mallctl("opt."name, (void *)&var1, &size, NULL, 0) == 0 && je_mallctl(altname, (void *)&var2, &size, NULL, 0) == 0) { emitter_kv_note(emitter, name, "opt."name, emitter_type, &var1, altname, emitter_type, &var2); }
...
#define OPT_WRITE_BOOL_MUTABLE(name, altname) OPT_WRITE_MUTABLE(name, bv, bv2, bsz, emitter_type_bool, altname)
...
#define OPT_WRITE_SSIZE_T_MUTABLE(name, altname) OPT_WRITE_MUTABLE(name, ssv, ssv2, sssz, emitter_type_ssize, altname)
