...
static int uidtb_fail;
static int gidtb_fail;
static int usrtb_fail;
static int grptb_fail;
...
#define FLUSHTB(arr, len, fail) do { if (arr != NULL) { for (i = 0; i < len; i++) if (arr[i] != NULL) free(arr[i]); arr = NULL; } fail = 0; } while (0);
