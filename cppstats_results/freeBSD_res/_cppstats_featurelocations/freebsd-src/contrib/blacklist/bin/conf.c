...
#define SIF_NAME(a) ((const struct sockaddr_if *)(const void *)(a))->sif_name
...
#define CMP(a, b, f) if ((a)->f > (b)->f) return -1; else if ((a)->f < (b)->f) return 1
...
#define CMP(a, b, f) if ((a)->f != (b)->f && (b)->f != FSTAR && (b)->f != FEQUAL) { if (debug > 1) (*lfun)(LOG_DEBUG, "%s: %s fail %d != %d", __func__, __STRING(f), (a)->f, (b)->f); return 0; }
