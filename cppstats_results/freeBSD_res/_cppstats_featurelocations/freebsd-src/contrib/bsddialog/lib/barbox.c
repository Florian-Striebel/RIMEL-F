...
draw_perc_bar(bar, 1, 1, w-8, perc, false, -1 );
wrefresh(bar);
...
draw_perc_bar(bar, 1, 1, w-8, mainperc, false, -1 );
...
if (i >= nminibar)
break;
if (minibar[i].status == 1)
break;

perc = pvconf->callback(&minibar[i]);

if (minibar[i].status == 5) {
minipercs[i] = 5;
update = true;
i++;
} else if (minibar[i].status == 1 || perc < 0) {
minipercs[i] = 1;
update = true;
} else if (perc == 0)
minipercs[i] = 7;
