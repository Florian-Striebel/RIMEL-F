...
reply = RedisModule_Call(ctx,"DEBUG","3cc" ,"PROTOCOL", "attrib");
...
RedisModule_Call(ctx,"DEL","c","myhash");
RedisModule_Call(ctx,"HSET","ccccc","myhash", "f1", "v1", "f2", "v2");
reply = RedisModule_Call(ctx,"HGETALL","3c" ,"myhash");
...
reply = RedisModule_Call(ctx,"DEBUG","3cc" ,"PROTOCOL", "null");
...
reply = RedisModule_Call(ctx,"DEBUG","3cc" ,"PROTOCOL", "double");
...
reply = RedisModule_Call(ctx,"DEBUG","3cc" ,"PROTOCOL", "bignum");
...
reply = RedisModule_Call(ctx,"DEBUG","3cc" ,"PROTOCOL", "verbatim");
...
reply = RedisModule_Call(ctx,"smembers","3c" ,"myset");
...
#define FAIL(msg, ...) { RedisModule_Log(ctx, "warning", "Failed NOTIFY Test. Reason: " #msg, ##__VA_ARGS__); goto err; }
...
#undef FAIL
#define FAIL(msg) { ok = 0; errString = msg; goto end; }
...
#define T(name,...) do { RedisModule_Log(ctx,"warning","Testing %s", name); reply = RedisModule_Call(ctx,name,__VA_ARGS__); } while (0)
