...
#define rbtn_black_height(a_type, a_field, a_rbt, r_height) do { a_type *rbp_bh_t; for (rbp_bh_t = (a_rbt)->rbt_root, (r_height) = 0; rbp_bh_t != NULL; rbp_bh_t = rbtn_left_get(a_type, a_field, rbp_bh_t)) { if (!rbtn_red_get(a_type, a_field, rbp_bh_t)) { (r_height)++; } } } while (0)
