...
#define assert(e) do { if (config_debug && !(e)) { malloc_write("<jemalloc>: Failed assertion\n"); abort(); } } while (0)
...
#define not_reached() do { if (config_debug) { malloc_write("<jemalloc>: Unreachable code reached\n"); abort(); } unreachable(); } while (0)
...
#define not_implemented() do { if (config_debug) { malloc_write("<jemalloc>: Not implemented\n"); abort(); } } while (0)
...
#define APPEND_C(c) do { if (i < size) { str[i] = (c); } i++; } while (0)





#define APPEND_S(s, slen) do { if (i < size) { size_t cpylen = (slen <= size - i) ? slen : size - i; memcpy(&str[i], s, cpylen); } i += slen; } while (0)
...
#define APPEND_PADDED_S(s, slen, width, left_justify) do { size_t pad_len = (width == -1) ? 0 : ((slen < (size_t)width) ? (size_t)width - slen : 0); if (!left_justify && pad_len != 0) { size_t j; for (j = 0; j < pad_len; j++) { APPEND_C(' '); } } APPEND_S(s, slen); if (left_justify && pad_len != 0) { size_t j; for (j = 0; j < pad_len; j++) { APPEND_C(' '); } } } while (0)
