...
#define CTL_PROTO(n) static int n##_ctl(tsd_t *tsd, const size_t *mib, size_t miblen, void *oldp, size_t *oldlenp, void *newp, size_t newlen);
#define INDEX_PROTO(n) static const ctl_named_node_t *n##_index(tsdn_t *tsdn, const size_t *mib, size_t miblen, size_t i);
CTL_PROTO(version)
CTL_PROTO(epoch)
...
#define MUTEX_STATS_CTL_PROTO_GEN(n) CTL_PROTO(stats_##n##_num_ops) CTL_PROTO(stats_##n##_num_wait) CTL_PROTO(stats_##n##_num_spin_acq) CTL_PROTO(stats_##n##_num_owner_switch) CTL_PROTO(stats_##n##_total_wait_time) CTL_PROTO(stats_##n##_max_wait_time) CTL_PROTO(stats_##n##_max_num_thds)
...
#define CHILD(t, c) sizeof(c##_node) / sizeof(ctl_##t##_node_t), (ctl_node_t *)c##_node, NULL
...
#define MUTEX_PROF_DATA_NODE(prefix) static const ctl_named_node_t stats_##prefix##_node[] = { {NAME("num_ops"), CTL(stats_##prefix##_num_ops)}, {NAME("num_wait"), CTL(stats_##prefix##_num_wait)}, {NAME("num_spin_acq"), CTL(stats_##prefix##_num_spin_acq)}, {NAME("num_owner_switch"), CTL(stats_##prefix##_num_owner_switch)}, {NAME("total_wait_time"), CTL(stats_##prefix##_total_wait_time)}, {NAME("max_wait_time"), CTL(stats_##prefix##_max_wait_time)}, {NAME("max_num_thds"), CTL(stats_##prefix##_max_num_thds)} };
...
#define OP(mtx) malloc_mutex_prof_merge( &(sdstats->astats.mutex_prof_data[ arena_prof_mutex_##mtx]), &(astats->astats.mutex_prof_data[ arena_prof_mutex_##mtx]));
MUTEX_PROF_ARENA_MUTEXES
#undef OP
if (!destroyed) {
...
#define READ_GLOBAL_MUTEX_PROF_DATA(i, mtx) malloc_mutex_lock(tsdn, &mtx); malloc_mutex_prof_read(tsdn, &ctl_stats->mutex_prof_data[i], &mtx); malloc_mutex_unlock(tsdn, &mtx);
...
#define READONLY() do { if (newp != NULL || newlen != 0) { ret = EPERM; goto label_return; } } while (0)
#define WRITEONLY() do { if (oldp != NULL || oldlenp != NULL) { ret = EPERM; goto label_return; } } while (0)
#define READ_XOR_WRITE() do { if ((oldp != NULL && oldlenp != NULL) && (newp != NULL || newlen != 0)) { ret = EPERM; goto label_return; } } while (0)
#define READ(v, t) do { if (oldp != NULL && oldlenp != NULL) { if (*oldlenp != sizeof(t)) { size_t copylen = (sizeof(t) <= *oldlenp) ? sizeof(t) : *oldlenp; memcpy(oldp, (void *)&(v), copylen); ret = EINVAL; goto label_return; } *(t *)oldp = (v); } } while (0)
#define WRITE(v, t) do { if (newp != NULL) { if (newlen != sizeof(t)) { ret = EINVAL; goto label_return; } (v) = *(t *)newp; } } while (0)
#define MIB_UNSIGNED(v, i) do { if (mib[i] > UINT_MAX) { ret = EFAULT; goto label_return; } v = (unsigned)mib[i]; } while (0)
#define CTL_RO_CLGEN(c, l, n, v, t) static int n##_ctl(tsd_t *tsd, const size_t *mib, size_t miblen, void *oldp, size_t *oldlenp, void *newp, size_t newlen) { int ret; t oldval; if (!(c)) { return ENOENT; } if (l) { malloc_mutex_lock(tsd_tsdn(tsd), &ctl_mtx); } READONLY(); oldval = (v); READ(oldval, t); ret = 0; label_return: if (l) { malloc_mutex_unlock(tsd_tsdn(tsd), &ctl_mtx); } return ret; }
...
#define CTL_RO_GEN(n, v, t) static int n##_ctl(tsd_t *tsd, const size_t *mib, size_t miblen, void *oldp, size_t *oldlenp, void *newp, size_t newlen) { int ret; t oldval; malloc_mutex_lock(tsd_tsdn(tsd), &ctl_mtx); READONLY(); oldval = (v); READ(oldval, t); ret = 0; label_return: malloc_mutex_unlock(tsd_tsdn(tsd), &ctl_mtx); return ret; }
#define CTL_RO_NL_CGEN(c, n, v, t) static int n##_ctl(tsd_t *tsd, const size_t *mib, size_t miblen, void *oldp, size_t *oldlenp, void *newp, size_t newlen) { int ret; t oldval; if (!(c)) { return ENOENT; } READONLY(); oldval = (v); READ(oldval, t); ret = 0; label_return: return ret; }
#define CTL_RO_NL_GEN(n, v, t) static int n##_ctl(tsd_t *tsd, const size_t *mib, size_t miblen, void *oldp, size_t *oldlenp, void *newp, size_t newlen) { int ret; t oldval; READONLY(); oldval = (v); READ(oldval, t); ret = 0; label_return: return ret; }
#define CTL_TSD_RO_NL_CGEN(c, n, m, t) static int n##_ctl(tsd_t *tsd, const size_t *mib, size_t miblen, void *oldp, size_t *oldlenp, void *newp, size_t newlen) { int ret; t oldval; if (!(c)) { return ENOENT; } READONLY(); oldval = (m(tsd)); READ(oldval, t); ret = 0; label_return: return ret; }
#define CTL_RO_CONFIG_GEN(n, t) static int n##_ctl(tsd_t *tsd, const size_t *mib, size_t miblen, void *oldp, size_t *oldlenp, void *newp, size_t newlen) { int ret; t oldval; READONLY(); oldval = n; READ(oldval, t); ret = 0; label_return: return ret; }
...
tsdn_t *tsdn = tsd_tsdn(tsd);
#define MUTEX_PROF_RESET(mtx) malloc_mutex_lock(tsdn, &mtx); malloc_mutex_prof_data_reset(tsdn, &mtx); malloc_mutex_unlock(tsdn, &mtx);
...
if (arena_ind < narenas_total_get() && (arena =
arena_get(tsd_tsdn(tsd), arena_ind, false)) != NULL) {
#if defined(JEMALLOC_GCC_ATOMIC_ATOMICS) || defined(JEMALLOC_GCC_SYNC_ATOMICS) || defined(_MSC_VER)
