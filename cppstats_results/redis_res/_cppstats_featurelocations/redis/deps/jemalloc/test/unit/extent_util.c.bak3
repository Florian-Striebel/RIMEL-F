...
#define TEST_UTIL_QUERY_EINVAL(a, b, c, d, why_inval) TEST_UTIL_EINVAL("query", a, b, c, d, why_inval)

#define TEST_UTIL_BATCH_EINVAL(a, b, c, d, why_inval) TEST_UTIL_EINVAL("batch_query", a, b, c, d, why_inval)


#define TEST_UTIL_VALID(node) do { assert_d_eq(mallctl("experimental.utilization." node, out, &out_sz, in, in_sz), 0, "Should return 0 on correct arguments"); assert_zu_eq(out_sz, out_sz_ref, "incorrect output size"); assert_d_ne(memcmp(out, out_ref, out_sz_ref), 0, "Output content should be changed"); } while (0)
...
#define TEST_EQUAL_REF(i, message) assert_d_eq(memcmp(out + (i) * 3, out_ref + (i) * 3, 3), 0, message)
